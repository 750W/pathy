    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
                ...
                match &self.cursor_mode {
                    CursorMode::Create => {
                        if let Some(pos) = resp.hover_pos() {
                            ...
                            console_log!("({}, {})", x, y);
                            self.points.push(BezPoint::new(x, y, 0.0, 0.0, 0.0, 0.0));
                        }
                    }
                    CursorMode::Edit => {
                        // TODO
            ...
            for point in &self.points {
                // Calculate points relative to screen
                let x = point.x * (self.scale as f32 / self.size) + rect.min.x;
                let y = point.y * (self.scale as f32 / self.size) + rect.min.y;
                ui.painter().circle_filled(pos2(x, y), 5.0, Color32::YELLOW);
            }
